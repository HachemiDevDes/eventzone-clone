...
export async function fetchAttendees(eventId = _activeEventId) {
  ...
  return (data || []).map(row => {
    ...
    if (sub && sub.answers && typeof sub.answers === 'object') {
      attendee.answers = { ...sub.answers, ...(attendee.answers || {}) };
      attendee.customAnswers = { ...sub.answers, ...(attendee.customAnswers || {}) };
      attendee.formAnswers = { ...sub.answers, ...(attendee.formAnswers || {}) };
      if (!attendee.company && (sub.answers.company || sub.answers.f_company)) {
        attendee.company = sub.answers.company || sub.answers.f_company;
      }
      if (!attendee.jobTitle && (sub.answers.jobTitle || sub.answers.job_title || sub.answers.f_job_title)) {
        attendee.jobTitle = sub.answers.jobTitle || sub.answers.job_title || sub.answers.f_job_title;
      }
...
function mapPendingFromDb(row) {
  ...
  return {
    ...
    company: meta.company || row.company || '',
    jobTitle: meta.jobTitle || row.job_title || '',
    ...
    answers: meta.answers || meta.customAnswers || {},
    formAnswers: meta.answers || meta.customAnswers || {},
  };
}
